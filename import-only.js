...
async function main() {
  console.log('🚀 Trade Portfolio Data Import Tool (Fixed)')
  console.log('==========================================')
  ...
  try {
    ...
    console.log('\n🎉 Import complete!')
    console.log('\nSummary:')
    const stocks = JSON.parse(fs.readFileSync('stocks_import_fixed.json', 'utf8'))
    const options = JSON.parse(fs.readFileSync('options_import_fixed.json', 'utf8'))
    console.log(`  - ${stocks.length} stocks`)
    console.log(`  - ${options.length} options (all CLOSED with P&L)`)
  } catch (err) {
